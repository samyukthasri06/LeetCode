...
class Solution {
    public List<TreeNode> generateTrees(int n) {
        ...
        for (int i = 0; i <= n; i++) {
            ...
            for (int j = 0; j <= n; j++) innerList.add(new ArrayList<>());
            dp.add(innerList);
        }

        for (int i = 1; i <= n; i++) dp.get(i).get(i).add(new TreeNode(i));
            ...
            for (int start = 1; start <= n - numberOfNodes + 1; start++) {
                ...
                for (int i = start; i <= end; i++) {
                    List<TreeNode> leftSubtrees = (i != start)
                        ...
                        : new ArrayList<>();
                    if (leftSubtrees.isEmpty()) leftSubtrees.add(null);
                    List<TreeNode> rightSubtrees = (i != end)
                        ? dp.get(i + 1).get(end)
                        : new ArrayList<>();
                    if (rightSubtrees.isEmpty()) rightSubtrees.add(null);
